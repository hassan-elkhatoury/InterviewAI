 ...
 */
public class reviewQuestionsController implements Initializable {
    ...
    private void loadIncorrectQuestions() {
        Integer activeCourseId = SessionContext.getActiveCourseId();
        int courseId = (activeCourseId != null) ? activeCourseId : 0;
        
        System.out.println("DEBUG: Active Chapter ID from SessionContext: " + courseId);
        
        if (courseId == 0) {
            System.err.println("ERROR: No active course set in SessionContext!");
            showAlert("No course Selected", "Please select a course from the dashboard first.");
            return;
        }

        try {
            questions  = questionDAO.getIncorrectQuestionsByCourseId(courseId);

            System.out.println("SUCCESS: Loaded " + questions.size() + "incorrect questions for course " + courseId);
            
            if (questions.isEmpty()) {
                System.out.println("WARNING: No incorrect questions found  for course" + courseId);
                showAlert("No Incorrecte Questions Available", 
                    "This course doesn't have any Incorrecte questions yet.");
            }
        } catch (SQLException e) {
            System.err.println("DATABASE ERROR: " + e.getMessage());
            e.printStackTrace();
            showAlert("Database Error", 
                "Failed to load incorrect questions from database.\nError: " + e.getMessage() + 
                "\n\nPlease check:\n1. Database connection\n2. course ID: " + courseId);
        }
    }
